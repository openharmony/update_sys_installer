...
int32_t SysInstallerManager::StartUpdateSingularPackageZip(const std::string &taskId,
    const std::vector<std::string> &pkgPath)
{
    if (helper_ == nullptr) {
        LOG(ERROR) << "helper_ null";
    ...
    return helper_->StartUpdateSingularPackageZip(taskId, pkgPath);
}
    ...
    uint64_t &createdSize, bool &isCreated)
{
    ...
    return helper_->CreateVabSnapshotCowImg(name, size, splitSize, createdSize, isCreated);
}
...
int32_t SysInstallerManager::ClearVabPatch()
{
    if (helper_ == nullptr) {
        LOG(ERROR) << "helper_ null";
        return -1;
