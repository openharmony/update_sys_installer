...
constexpr const char *ARK_WEB_BUNDLE_NAME = "com.ohos.nweb";
...
bool ParseHmpVersionInfo(const JsonNode &package, ModulePackageInfo &versionInfo)
{
    ...
    versionInfo.hotApply = Utils::String2Int<int>(hotApply.value());
    ...
    if (versionInfo.type == HMP_APP_TYPE || versionInfo.type == HMP_MIX_TYPE) {
        ...
        versionInfo.apiVersion = Utils::String2Int<int>(apiVersion.value());
...
{
    LOG(INFO) << "IsHot: hotApply= " << versionInfo_.hotApply << " ;type= " << versionInfo_.type;
    if (versionInfo_.hotApply != 1) {
    ...
    if (versionInfo_.type == HMP_APP_TYPE && versionInfo_.bundleInfoList.size() == 1 &&
        strcmp(versionInfo_.bundleInfoList.front().bundleName.c_str(), ARK_WEB_BUNDLE_NAME) == 0) {
        return HOT_APP_TYPE;
    }
    if ((versionInfo_.type == HMP_SA_TYPE || versionInfo_.type == HMP_SA_TYPE_OLD) &&
        versionInfo_.saInfoList.size() == 1 && IsHotSa(versionInfo_.saInfoList.front().saId)) {
        return HOT_SA_TYPE;
    }
    return COLD_MIX_TYPE;
}
