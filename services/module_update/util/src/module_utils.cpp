...
int GetDeviceApiVersion(void)
{
    ...
    return Utils::String2Int<int>(apiVersion);
}
...
void KillProcessOnArkWeb(void)
{
    ...
    if (appMgrClient == nullptr) {
        LOG(ERROR) << "appMgrClient is nullptr, kill ark web failed.";
        return;
    }
    LOG(INFO) << "Start to kill ark web.";
    appMgrClient->KillProcessDependedOnWeb();
}

bool InstallHmpBundle(const std::string &hmpPath, bool revert)
{
    auto bmsMgrClient = DelayedSingleton<AppExecFwk::BundleMgrClient>::GetInstance();
    if (bmsMgrClient == nullptr) {
        LOG(ERROR) << "bmsMgrClient is nullptr.";
        return false;
    }
